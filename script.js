...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- 11. Contact Form ---
    if (contactForm) {
        const formResponse = select('#formResponse');
        const submitButton = select('#contactForm button[type="submit"]');
        const loader = select('#contactForm .loader');
        const buttonText = select('#contactForm .btn-text');

        const validateEmail = (email) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(String(email).toLowerCase());

        const showFieldError = (input, message) => {
            input.classList.add('is-invalid'); // Bootstrap class
            const group = input.closest('.form-group'); // Bootstrap structure
            if (group) {
                let errorElement = group.querySelector('.invalid-feedback, .error-message'); // Check both
                if (!errorElement) { // Create if not found
                   errorElement = document.createElement('div');
                   errorElement.className = 'invalid-feedback'; // Use Bootstrap class
        ...
        const clearFieldError = (input) => {
            input.classList.remove('is-invalid'); // Bootstrap class
            const group = input.closest('.form-group');
            if (group) {
                const errorElement = group.querySelector('.invalid-feedback, .error-message');
                if (errorElement) {
        ...
        contactForm.addEventListener('input', (e) => {
            if (e.target.required) clearFieldError(e.target);
        });

        contactForm.addEventListener('submit', async (e) => {
            e.preventDefault();
            formResponse.textContent = ''; formResponse.className = '';
            let isValid = true;

            // Client-Side Validation
            const requiredFields = selectAll('#contactForm [required]');
            requiredFields.forEach(input => {
                clearFieldError(input); // Clear previous errors
                if (!input.value.trim()) {
                    isValid = false;
                    showFieldError(input, `${input.previousElementSibling?.textContent || input.name || 'This field'} is required.`);
            ...
            if (!isValid) {
                 formResponse.textContent = 'Please fix the errors above.';
                 formResponse.className = 'error';
                return;
            }

            // Show Loading State
            if (submitButton && loader && buttonText) {
                submitButton.disabled = true; loader.style.display = 'inline-block'; buttonText.style.display = 'none';
            }
            formResponse.textContent = 'Sending...'; formResponse.className = 'sending';
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    contactForm.reset();
                    requiredFields.forEach(clearFieldError); // Clear validation states
                } else {
                    // Handle server-side errors (e.g., from Formspree)
                    let errorMsg = 'An error occurred on the server. Please try again.';
                    try { // Try to parse Formspree error
                        const data = await response.json();
                        if (data && data.errors) {
                           errorMsg = data.errors.map(err => err.message).join(', ');
                        }
                    } catch (parseError) { /* Ignore parsing error, use generic message */ }
                    formResponse.textContent = errorMsg;
                    formResponse.className = 'error';
                }
            } catch (networkError) {
                console.error('Form submission network error:', networkError);
                formResponse.textContent = 'Network error. Please check connection or try again.';
                formResponse.className = 'error';
            } finally {
                // Restore button state
                if (submitButton && loader && buttonText) {
                    submitButton.disabled = false; loader.style.display = 'none'; buttonText.style.display = 'inline-block';
                }
            }
        });
    }
